...
-
    *[Greenstadt, J. (1970). Variations on variable-metric methods.(With discussion). Mathematics of Computation, 24(109), 1-22.](https://web.archive.org/web/20170817130143id_/http://www.ams.org/journals/mcom/1970-24-109/S0025-5718-1970-0258248-4/S0025-5718-1970-0258248-4.pdf)*:
...
- $B_{t}$ that satisfies the secant equation $B_{t}\mathbf{s}_t=\mathbf{y}_t$
...
(\mathbf{y}_t - B_{t-1}\mathbf{s}_t)^\top = \alpha \mathbf{s}_{t}^\top\mathbf{u}\mathbf{u}^\top$$
...
We can also do the same for Hessian inverse. $B_{t}\mathbf{s}_t=\mathbf{y}_t$, therefore $B_{t}^{-1}\mathbf{y}_t=\mathbf{s}_t$. We can repeat the entire derivation above, simply swapping $\mathbf{s}_t$ and $\mathbf{y}_t$, and we get a formula which updates Hessian inverse approximation:
$$
...
![Style Transfer benchmark](/assets/quasi-newton/style-transfer)
_Style Transfer benchmark_

It has also been [widely used for Physics-inspired neural networks](https://arxiv.org/html/2405.04230v3), although recently [SOAP has been shown to outperform it](http://arxiv.org/html/2502.00604v1).
![PINN benchmark](/assets/quasi-newton/pinn)
_PINN benchmark_
...
![QN on rosenbrock](/assets/quasi-newton/rosen)
