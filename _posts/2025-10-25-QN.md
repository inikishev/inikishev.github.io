...
{::nomarkdown}$$
B_{t}^{-1} \leftarrow B_{t-1}^{-1} + \frac{(\mathbf{s}_k^T \mathbf{y}_k + \mathbf{y}_k^T B_{t-1}^{-1} \mathbf{y}_k) \mathbf{s}_k \mathbf{s}_k^T}{(\mathbf{s}_k^T \mathbf{y}_k)^2} - \frac{B_{t-1}^{-1} \mathbf{y}_k \mathbf{s}_k^T + \mathbf{s}_k \mathbf{y}_k^T B_{t-1}^{-1}}{\mathbf{s}_k^T \mathbf{y}_k}
$${:/nomarkdown}
...
{::nomarkdown}$$x_{t+1} \leftarrow x_t - \gamma_t  B_t^{-1} \mathbf{g}(\mathbf{x}_t)^\text{T}$${:/nomarkdown}
...
Diagonal QN can't use the secant equation. If we take the secant equation {::nomarkdown}$B\mathbf{s}=\mathbf{y}${:/nomarkdown} and constrain {::nomarkdown}$B${:/nomarkdown} to be diagonal, the only solution is {::nomarkdown}$B = \mathbf{y}/\mathbf{s}${:/nomarkdown}, i.e. element-wise of {::nomarkdown}$\mathbf{y}${:/nomarkdown} by {::nomarkdown}$\mathbf{s}${:/nomarkdown}. This is extremely unstable and basically doesn't work.
